    ...
    fn end_snapshot(&self, modified_tasks: Vec<Vec<TaskId>>) {
        ...
        for entry in self.snapshots.iter() {
            ...
            if let Some(mut inner) = self.map.get_mut(&key) {
                let was_already_modified = inner.flags.any_modified();
                ...
                // It is possible that modification raced with this loop so back off if it was
                // already modified
                if !was_already_modified {
...
impl StorageWriteGuard<'_> {
    ...
    fn track_modification_internal(
        ...
        // Tasks without a persistent_task_type cannot be meaningfully serialized,
        // so there's no point tracking them as modified for persistence.
        ...
        match (self.storage.snapshot_mode(), modified) {
            ...
            (true, true) => {
                ...
                if !flags.any_snapshot() {
                    // Snapshot all non-transient fields but keep the modified and
                    // new_persistent_task bits (which are transient but needed for
                    // correct snapshot processing in encode_snapshot_item).
    ...
    PS: Fn(TaskId, Box<TaskStorage>, &mut TurboBincodeBuffer) -> SnapshotItem + Sync,
{
    fn next_item(&mut self) -> Option<SnapshotItem> {
        ...
        if let Some(task_id) = self.modified.pop() {
