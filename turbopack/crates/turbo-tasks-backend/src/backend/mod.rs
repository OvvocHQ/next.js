            ...
            |task_id: TaskId,
             ...
             buffer: &mut TurboBincodeBuffer| {
                ...
                SnapshotItem {
                    ...
                    task_type_hash: if inner.flags.new_persistent_task() {
                        let tt = inner.get_persistent_task_type();
                        debug_assert!(
                            tt.is_some(),
                            "new_persistent_task is set but persistent_task_type is None for task \
                             {task_id:?}"
                        );
                        tt.map(|tt| compute_task_type_hash(tt))
                    } else {
                        None
                    },
                }
