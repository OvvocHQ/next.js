...
async function createNonTaskyPrefetchResponseStream(
  ...
  // Re-enqueue all the chunks into a new stream. We don't need to
  // concatenate them into a single chunk — we just need to ensure all data
  // is available before Flight starts reading. When a ReadableStream already
  // has data in its internal queue, reader.read() resolves as a microtask,
  // not a macrotask. Microtasks run to completion before yielding to the
  // event loop, so Flight will process all chunks in one synchronous
  // microtask chain. This guarantees that all thenables (like vary params)
  // are resolved before any subsequent code observes them.
